...
class ProgressDisplay:
    ...
    def show_complete(self, reddit_count: int, x_count: int):
        ...
        if IS_TTY:
            sys.stderr.write(f"\n{Colors.GREEN}{Colors.BOLD}[OK] Research complete{Colors.RESET} ")
            ...
            sys.stderr.write(f"[OK] Research complete ({elapsed:.1f}s) - Reddit: {reddit_count} threads, X: {x_count} posts\n")
        sys.stderr.flush()
    ...
    def show_cached(self, age_hours: float = None):
        if age_hours is not None:
            ...
            age_str = ""
        sys.stderr.write(f"{Colors.GREEN}[CACHED]{Colors.RESET} {Colors.DIM}Using cached results{age_str} - use --refresh for fresh data{Colors.RESET}\n\n")
        sys.stderr.flush()
    ...
    def show_web_only_complete(self):
        ...
        elapsed = time.time() - self.start_time
        if IS_TTY:
            sys.stderr.write(f"\n{Colors.GREEN}{Colors.BOLD}[OK] Ready for web search{Colors.RESET} ")
            sys.stderr.write(f"{Colors.DIM}({elapsed:.1f}s){Colors.RESET}\n")
            sys.stderr.write(f"  {Colors.GREEN}Web:{Colors.RESET} Claude will search blogs, docs & news\n\n")
        else:
            sys.stderr.write(f"[OK] Ready for web search ({elapsed:.1f}s)\n")
        sys.stderr.flush()
